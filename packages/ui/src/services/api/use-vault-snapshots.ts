...
import { ChartRangeTypes, SnapshotData } from "./types/snapshot";
...
export function useSnapshots({
  ...
  return useQuery<SnapshotData[]>({
    ...
    queryFn: async ({ signal }) => {
      ...
      const res = await fetch(`${API_GATEWAY}/lagoon/snapshots/test?${params}`, {
        signal,
    ...
    refetchInterval: () => {
      ...
      if (ranges === "all" || ranges === "1y" || ranges === "6m" || ranges === "1m") return 60_000;
      return false;
    },
