import ERC20Abi from "@/lib/protocols/abis/IERC20.json";
...
export const DEFAULT_EXPIRATION = Math.floor(Date.now() / 1000) + 60 * 60 * 24 * 365 * 100; // 100 years
...
/* export async function getERC20Signature({
  ...
  const { domain, types, values } = AllowanceTransfer.getPermitData(permit, token, chainId);
