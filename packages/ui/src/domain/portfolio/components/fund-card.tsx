...
export default function FundCard({
  ...
  return (
    <div
      ...
      onClick={() => {
        navigate({ to: `/fund-operate?vaultId=${vault.id}` });
      }}
    >
            ...
            {false &&
              isMobile && ( // TODO: Fund actions momentarily disabled
                <div className="flex flex-row gap-4 mt-8">
                  <Deposit vault={vault} className="w-full !h-10" isLoading={isLoading} />
                  <Withdraw vault={vault} className="w-full !h-10" isLoading={isLoading} />
                </div>
              )}
          </div>
        </div>
