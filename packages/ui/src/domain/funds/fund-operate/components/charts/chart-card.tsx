import { ChartRangeTypes } from "@/services/api/types/snapshot";
      ...
      <div className={variant === "large" ? containerLarge : containerSmall}>
        <div className="mb-1">
          <div className="flex items-center w-full">
            {title && <h3 className="text-lg font-semibold justify-start w-full">{title}</h3>}
            {externalToggle && (
            ...
            {selector && (
              <div className="flex items-center gap-2 justify-end w-full">{selector}</div>
            )}
          </div>

          {subtitle && <p className="text-sm text-muted-light dark:text-muted">{subtitle}</p>}
        </div>

        <div className={variant === "large" ? containerBodyLarge : containerBodySmall}>
