...
export function useDeposit() {
  ...
  //const { safeAddress, isLoading, error, executeSafeTransaction } = useSafeLinkedAccountContext();
  ...
  const cancelDepositRequest = async (vaultAddress: string) => {
    ...
    const cancelDepositRequestCall = {
      to: vaultAddress,
      value: "0",
      data: new Contract(vaultAddress, VaultABI, signer).interface.encodeFunctionData(
